...
function build_reusable_model_uncached(
    ...
    v_e = map(_ -> @variable(m, lower_bound = -1, upper_bound = 1), input_range) # perturbation added
    v_x0 = map(_ -> @variable(m, lower_bound = 0, upper_bound = 1), input_range) # perturbation + original image
...
function build_reusable_model_uncached(
    ...
    input_range = CartesianIndices(size(input))
    v_e = map(_ -> @variable(m, lower_bound = -pp.norm_bound, upper_bound = pp.norm_bound), input_range) # perturbation added
    v_x0 = map(i -> @variable(m, lower_bound = max(0, input[i] - pp.norm_bound), upper_bound = min(1, input[i] + pp.norm_bound)), input_range) # perturbation + original image
